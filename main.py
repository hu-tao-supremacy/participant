...
class ParticipantService(participant_service_grpc.ParticipantServiceServicer):
    ...
    def SubmitAnswerForPostEventQuestion(self, request, context):
        try:
            ...
            user_event = session.query(UserEvent).filter(
                UserEvent.id == user_event_id).first()
            ...
            data = map(lambda result: common.Answer(id=result.id, user_event_id=result.user_event_id,
        ...
        try:
            ...
            if (user_events_by_event_id):
                ...
                if users:
                    ...
                    return participant_service.GetApprovedUserByEventIdResponse(users=approved_user)
